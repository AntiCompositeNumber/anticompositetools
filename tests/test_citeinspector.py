...
def test_get_citoid_template_types():
    s = requests.Session()
    template_type_map, supported_templates = citeinspector.get_citoid_template_types(s)
...
def test_concat_items():
    ...
    citoid_data = {
        "data": {"last": "Ipsum", "first": "Lorem", "title": "Foo Bar"},
        "template": "Cite book",
        "source": "WorldCat",
        "template_data": {
            "params": {
    ...
    concat_data = {
        ...
        "location": "ref",
        "data": {
            "last1": {"wikitext": "Ipsum", "citoid": "Ipsum", "ratio": 100},
            "first1": {"wikitext": "Lorem", "citoid": "Lorem", "ratio": 100},
            "title": {"wikitext": "", "citoid": "Foo Bar", "ratio": 0},
            "isbn": {"wikitext": "9781786751041", "citoid": "", "ratio": 0},
        },
        "wikitext": "foo",
    }
...
def test_fuzz_item():
    assert citeinspector.fuzz_item("The Quick Brown Fox", "The Fantastic Mr. Fox") == 53
...
def test_map_citoid_to_templates():
    raw_citoid_data = {
        "ISBN": ["978-1-78675-104-1", "1-78675-104-6"],
        "author": [["", "Carroll, Lewis, 1832-1898,"], ["", "Ipsum, Lorem, 1970,"]],
        "editor": [["Dolor", "Sit"], ["Foo", "Bar"]],
        "itemType": "book",
        "source": ["WorldCat"],
        "title": "Alice's adventures in Wonderland",
    }
    wikitext_data = {"location": "ref", "name": "fcabdd30-69ab-4a76-896d-982a4d61f6ca"}
    template_data_cache = {}
    template_type_map = {"book": "Cite book"}
    session = requests.Session()

    citoid_data, new_td_cache = citeinspector.map_citoid_to_templates(
        raw_citoid_data, wikitext_data, template_data_cache, template_type_map, session
    )
...
def test_get_retry_badmethod():
    with pytest.raises(NotImplementedError):
        citeinspector.get_retry(
            "http://example.com", requests.Session(), method="delete"
        )
